...
export class Neo4jGraphStore implements GraphStore {
  ...
  async getRelationships(query: GraphQuery): Promise<Relationship[]> {
    ...
    try {
      ...
      let rels = result.records.map((record) => {
  ...
  async upsertRelationshipBatch(upserts: RelationshipUpsert[]): Promise<void> {
    if (upserts.length === 0) return;
    const session = this.driver.session();
    try {
      await session.run(
        `UNWIND $upserts AS u
         MERGE (a:Agent {id: u.fromId})
         MERGE (b:Agent {id: u.toId})
         MERGE (a)-[r:RELATES {type: u.relType}]->(b)
         ON CREATE SET r.strength = u.inc, r.since = u.tick, r.lastInteraction = u.tick, r.metadata = '{}'
         ON MATCH SET r.strength = CASE WHEN r.strength + u.inc > 1.0 THEN 1.0 ELSE r.strength + u.inc END, r.lastInteraction = u.tick`,
        {
          upserts: upserts.map(u => ({
            fromId: u.from,
            toId: u.to,
            relType: u.type,
            inc: u.strengthIncrement,
            tick: u.tick,
          })),
        },
      );
